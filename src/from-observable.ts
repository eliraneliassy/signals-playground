...
import {computed, DestroyRef, inject, SettableSignal, signal, Signal} from '@angular/core';
...
export function fromObservable<T, U = never>(source: Observable<T>, initialValue?: U): Signal<T|U> {
  ...
  let state: SettableSignal<State<T|U>>;
  if (initialValue === undefined && arguments.length !== 2) {
    ...
    state = signal({kind: StateKind.Value, value: initialValue!});
  }
